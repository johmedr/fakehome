...
class HHDataset(object):
    ...
    def __init__(self, dataset_name=None,  ontology_builder=None):
        ...
        if dataset_name is None and ontology_builder is None:
            raise AttributeError(
                "Need to provide either dataset_name or ontology_builder !")
            ...
            if not isinstance(ontology_builder, OntologyBuilder):
                raise AttributeError(
                    "ontology_builder must be an instance of OntologyBuilder !")
        ...
        self._datetime_to_list = lambda date: [
            date.month, date.day, date.hour, date.minute, date.second, date.microsecond]

    def _time_embedding(self, measurement_list):
        return np.array(
            [
                self._datetime_to_list(datetime.datetime.strptime(
    ...
    def _input_embedding(self, measurement_list, normalize=True):
        ...
        if normalize:
            input_embedding = np.nan_to_num(
                input_embedding / np.max(np.abs(input_embedding)))
    ...
    def get_training_inputs(self, window_size=100, starting_line=0, normalize=True):
        raw_slice = self.ontology_builder.read_training_data(
            window_size=window_size, starting_line=starting_line)
        I = self._input_embedding(
            raw_slice['sensor_events'], normalize=normalize)
        D = self._events_to_nodes_translmatrix(raw_slice['sensor_events'])
        return D.dot(I)  # ??? Here some attention ?

# def get_training_outputs(self, window_size=100, starting_line=0,
# normalize=True):
