...
class SimpleDataFetcher(object):
    ...
    def __init__(self, data_path=ROOT_PATH, dataset_name="hh101"):
        ...
        self.dataset_path = os.path.join(
            data_path, os.path.join(dataset_name, dataset_name))
        ...
        def extract_sensor_list():
            ...
            with open(self.anndata_filepath) as file:
                line_pattern = re.compile(
                    '^[0-9]{4}-[0-9]{2}-[0-9]{2}\s[0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]+\s([A-Z]{1,2})([A-Z0-9]{3,6})*')
                ...
                for line in file:
                    ...
                    if not tokens:
                        warnings.warn(
                            "Unable to find data pattern on line %s" % line)
        ...
        self.activity_dict = {ACTIVITY_LIST[
            i]: i for i in range(self.activity_nb)}
    ...
    def read_annotated_data(self, window_size=-1, starting_line=0):
        ...
        with open(self.anndata_filepath) as file:
            ...
            if window_size == -1:
                ...
                lines = file.readlines()[starting_line:]
                warnings.warn("Trying to read %d lines, but annotated data file contains %d lines." % (
            ...
            for line in lines:
                ...
                # Translate values: either state or real value
                ...
                if tokens.group(6):
                    ...
                    # 2 activity formats: either single 'name state' or 'name'
                    # - manage first case here
                    if act_tokens.group(3):
                        ...
                        if activity_state == 'end' and activity_name in self.waiting_activities:
                            ...
                            self.waiting_activities.add(activity_name)
                            warnings.warn("Unseen activity '%s'" %
                                          activity_name)

                    # Manage second case here
                    else:

                        activity_events[self.activity_dict[
                            activity_name], line_num] = 1

                # Link previous state here
